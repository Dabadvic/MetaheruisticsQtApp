 ...
 * @param seed      Semilla usada para la generación de aleatorios
 * @param useGiven  Indica si se usa la solucion dada como inicial o una aleatoria
 * @param solutionsToCheck  Cantidad de soluciones a comprobar
 */
LocalSearch::LocalSearch(const vector<vector<int> > &distances,const vector<vector<int> > &flow,const vector<int> &solution, int seed, bool useGiven, int solutionsToCheck)
{
 ...
 */
int LocalSearch::search()
{
    //Genera solución inicial aleatoria o usa la recibida (ya en la variable)
    if (!useGiven_)
    {
...
void LocalSearch::setSolution(const vector<int> &solution)
{
    solution_ = solution;
    calculateCost();
}

 ...
 * @param flow          Matriz de flujos
 * @param type          Tipo de cruce para AGG, 0 para posición y 1 para OX
 * @param am_type       Tipo de memético, 0 para 10,1.0; 1 para 10,0.1; y 2 para 10,0.1mej
 * @param seed          Semilla aleatoria para controlar la ejecución
 * @param evaluations   Cantidad de evaluaciones de la función objetivo a realizar
 * @param population    Tamaño de la población
 * @param crossProbability      Probabilidad de cruce
 * @param mutationProbability   Probabilidad de mutación
 */
Agg::Agg(const vector<vector<int> > &distances, const vector<vector<int> > &flow, int type, int am_type, int seed, int evaluations, int population, float crossProbability, float mutationProbability)
{
...
/**
 * @brief compareChromosome
 * @param a
 * @param b
 * @return
 *
 * Función de comparación de cromosomas
 */
bool compareChromosome(Chromosome a, Chromosome b)
{
...
void Agg::search()
{
    ...
    while (evaluations < evaluations_)
    {
        ...
        //Apartado de hibridación: Aplicar búsqueda local
        //Cada 10 generaciones, se aplica el método
        if (generationCounter == 10)
        {
            generationCounter = 1;
            LocalSearch localsearch(distances_, flow_, generation_[0].solution, seed_, true, 400);
            if (am_type_ == 0)
            {
                //AM tipo 10,1.0
                for (int j=0;j<(int)generation_.size();j++)
                {
                    localsearch.setSolution(generation_[j].solution);
                    evaluations+= localsearch.search();
                    generation_[j].solution = localsearch.getSolution();
                    generation_[j].cost = localsearch.getCost();
                }
            }
            else if (am_type_ == 1)
            {
                //AM tipo 10,0.1
                //int sub = (int)(population_ * 0.1);
                int chosen;
                for (int j=0;j<(int)generation_.size();j++)
                {
                    chosen = rand() % 100;
            ...
            else
            {
                //AM tipo 10,0.1mej
                int sub = (int)(population_ * 0.1);
                sort(generation_.begin(), generation_.end(), compareChromosome);
                for(int j=0;j<sub;j++)
                {
                    localsearch.setSolution(generation_[j].solution);
                    evaluations+= localsearch.search();
                    if (localsearch.getCost() < generation_[j].cost)
                    {
                        generation_[j].solution = localsearch.getSolution();
                        generation_[j].cost = localsearch.getCost();
                    }
                }
                random_shuffle(generation_.begin(), generation_.end());
            }
        }
    }
}
